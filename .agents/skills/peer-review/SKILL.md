...
Peer review is itself a research act. The rules in `CLAUDE.md` / `AGENTS.md` → *Operating rules* bind the reviewer:
...
- **Rule 10** (objective report voice) — write the review as a standalone verdict on the paper, with no requester references in the report prose.

## Warrant Labels (Project Standard)

Every load-bearing claim made *by this review* (not by the paper) carries a warrant per `CLAUDE.md` / `AGENTS.md`:

| Label | Meaning |
|---|---|
| `(traced)` | Followed evidence chain to a primary source fetched in this session. State URL + access date. |
| `(deferred to consensus)` | Relying on a named consensus mechanism, such as field standards, a methods guideline, or a peer-reviewed literature body. Name the body. |
