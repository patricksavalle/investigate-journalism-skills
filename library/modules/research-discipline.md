## Research Discipline (CLAUDE.md/AGENTS.md)

{{intro}}

{{rule0}}
- **Rule 1** (pre-search hypothesis registration) — {{rule1}}
- **Rule 2** (steelman from primary literature) — {{rule2}}
- **Rule 3** (primary before secondary) — {{rule3}}
- **Rule 4** (map institutional networks) — {{rule4}}
- **Rule 5** (Tier 0 priority for time-sensitive claims) — {{rule5}}
- **Rule 6** (bias self-audit) — {{rule6}}
- **Rule 7** (minimum search volumes) — {{rule7}}
- **Rule 8** (hostility check on sources) — {{rule8}}
- **Rule 9** (interactive refinement: user contributions are inputs, not warrants) — {{rule9}}
- **Rule 10** (objective report voice) — {{rule10}}
