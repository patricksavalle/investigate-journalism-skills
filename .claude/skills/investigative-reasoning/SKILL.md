...
- `intuitive-thinking` - when an investigation begins from an anomaly signal, gut feeling, or pattern match. Capture the hunch before building hypotheses.
...
Eighteen named patterns with red-flag indicators and search templates:
[`references/patterns-and-deep-research.md`](references/patterns-and-deep-research.md).
**Open it here.**

Per match: does the event structure match, does the narrative management match,
which actor would have deployed it? Feed matches to Phases 4 and 5. Three or more
patterns pointing at one actor is a Hypothesis B *trigger*, not confirmation.
Construct the most plausible ordinary explanation for each match — incompetence,
coincidence, confirmation bias — and if it is equally parsimonious, drop the flag.
...
Five passes — horizon scan, deep dive, financial mapping, gap and silence,
synthesis — plus the public database list:
[`references/patterns-and-deep-research.md`](references/patterns-and-deep-research.md).
